...
bool PGM_BASE::InitPgm( bool aHeadless, bool aSkipPyInit, bool aIsUnitTest )
{
    ...
    // cicada: 恢复上游 :454 的 settings 分配（wave2 剪版把整块删了 → m_settings_manager 恒 null
    // → GetSettingsManager() 返回空引用 → GetAppSettings 的 std::lock_guard 在垃圾地址崩）。
    // 只分配壳层需要的 settings 管理器（library/backgrounds/notifications 保持 null，路径未触及）。
