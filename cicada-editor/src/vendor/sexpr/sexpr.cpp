...
namespace SEXPR
{
    ...
    std::string SEXPR::AsString( size_t aLevel ) const
    {
        ...
        else if( IsDouble() )
        {
            ...
            // Keep scientific notation out of schematic output.
            std::ostringstream stream;
            stream << std::setprecision( 10 ) << GetDouble();
            out = stream.str();
            ...
            if( out.find("e") != std::string::npos )
            {
                ...
                fixed << std::fixed << std::setprecision( 10 ) << GetDouble();
                out = fixed.str();
	...
	size_t SEXPR_LIST::doScan( const SEXPR_SCAN_ARG *args, size_t num_args )
	{
		...
		for( i = 0; i < num_args; i++ )
		{
			...
			try
			{
				...
				{
					*arg.u.int_value = child->GetInteger();
				}
